...
let currency = "MAD";
let language = "ar";
const currencyRates = { MAD: 1, EUR: 0.095 }; // مثال التحويل
...
function renderProducts() {
    ...
    products.forEach(p => {
        ...
        card.innerHTML = `
            <img src="${p.img}" alt="${p.name[language]}">
...
document.getElementById('orderWhatsApp').onclick = () => {
    ...
    cart.forEach((item, idx) => {
        text += `${idx+1}- ${item.name[language]} - ${(item.discountPrice*currencyRates[currency]).toFixed(2)} ${currency}%0A`;
    });
